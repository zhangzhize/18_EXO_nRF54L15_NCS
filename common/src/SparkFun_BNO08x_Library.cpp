...
static const bool _use_int_sem = true;
                   ...
                   const struct gpio_dt_spec *rst_dev)
{
  ...
  if (_rst_dev != NULL)
  {
    if (!device_is_ready(_rst_dev->port))
      return false;
    if (gpio_pin_configure_dt(_rst_dev, GPIO_OUTPUT_INACTIVE) != 0)
      return false;
  }
  if (_int_dev != NULL)
  {
    if (!device_is_ready(_int_dev->port))
      return false;
    if (gpio_pin_configure_dt(_int_dev, GPIO_INPUT) != 0)
      return false;
    ...
    if (_use_int_sem)
    {
      ...
      if (gpio_add_callback(_int_dev->port, &_int_cb_data) != 0)
      {
        return false;
      }
    }
