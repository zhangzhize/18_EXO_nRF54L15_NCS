...
static void stuffMetadata(sh2_SensorMetadata_t *pData, uint32_t *frsData)
{
  ...
  // Copy vendor id
  memcpy(pData->vendorId, (uint8_t *)&frsData[vendorIdOffset], pData->vendorIdLen);
}
...
{
  int status = sendCmd(pSh2, pSh2->opData.sendCmd.req.command, pSh2->opData.sendCmd.req.p);
