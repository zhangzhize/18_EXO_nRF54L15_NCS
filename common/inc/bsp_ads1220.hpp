/**
 * @file bsp_ads1220.hpp
 * @author Zhangzhize (you@domain.com)
 * @brief port of Wolfgang (Wolle) Ewald's SparkFun ADS122C04 Arduino library to Zephyr RTOS for use with ADS1220
 * @version 0.1
 * @date 2026-05-24
 *
...
{
  ADS1220_DR_LVL_0 = 0x00,  // default, 20SPS
  ADS1220_DR_LVL_1 = 0x20,  // 45SPS
  ADS1220_DR_LVL_2 = 0x40,  // 90SPS
  ADS1220_DR_LVL_3 = 0x60,  // 175SPS
  ADS1220_DR_LVL_4 = 0x80,  // 330SPS
  ADS1220_DR_LVL_5 = 0xA0,  // 600SPS
  ADS1220_DR_LVL_6 = 0xC0  // 1000SPS
} ads1220DataRate;
...
public:
  ...
  explicit ADS1220(void) {};
  ~ADS1220() = default;

  bool begin(const struct spi_dt_spec *spi_spec, const struct gpio_dt_spec *drdy_gpio_spec = NULL);
  ...
  void setRefp0Refn0AsVrefAndCalibrate();
  void setRefp1Refn1AsVrefAndCalibrate();
  void setIntVRef();
  ...
  /* Application-controlled DRDY synchronization. */
...
protected:
  ...
  const struct spi_dt_spec *spi_spec_;
  const struct gpio_dt_spec *drdy_gpio_spec_;
  struct drdy_context drdy_ctx_;

  static void drdy_isr_handler(const struct device *dev, struct gpio_callback *cb, uint32_t pins);

  uint8_t reg_value_ = 0x00;
  float v_ref_ = 3.3f;
  uint8_t gain_ = 128;
  bool ref_measurement_ = false;
  bool do_not_bypass_PGA_if_possible_ = false;
  bool non_blocking_mode_ = false;
  ads1220ConvMode conv_mode_ = ADS1220_SINGLE_SHOT;
